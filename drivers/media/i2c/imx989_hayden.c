...
static const struct regval imx989_linear_10bit_1920x1080_30fps_pd_on[] = { 
			    ...
			    int len, u32 val) //ok for all
{
	...
	if (reg == 0x0100) {
		dev_info(&client->dev,
			 "IMX989_REG_CTRL_MODE register 0x%04x: 0x%08x 0x%08x\n",
			 reg, val, read_val);
	}
...
static int __imx989_power_on(struct imx989 *imx989)//ok for all
{
	...
	else
	{
		...
		if (clk_get_rate(imx989->xvclk) != IMX989_XVCLK_FREQ)
			dev_warn(dev, "xvclk mismatched, modes are based on 19.2MHz, actual: %lu\n",
				 clk_get_rate(imx989->xvclk));
...
static int imx989_set_ctrl(struct v4l2_ctrl *ctrl)//FIXME: check 989's exposure and gain registers
{
	...
	case V4L2_CID_TEST_PATTERN:
		dev_info(&client->dev, "set testpattern 0x%x\n",
			ctrl->val);
